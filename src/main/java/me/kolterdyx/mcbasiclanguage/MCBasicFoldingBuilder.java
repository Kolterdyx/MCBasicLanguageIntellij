...
import me.kolterdyx.mcbasiclanguage.MCBasicAnnotator;
...
final class MCBasicFoldingBuilder extends FoldingBuilderEx implements DumbAware {
  ...
  public FoldingDescriptor @NotNull [] buildFoldRegions(@NotNull PsiElement root,
    ...
    FoldingGroup group = FoldingGroup.newGroup(MCBasicAnnotator.SIMPLE_PREFIX_STR);
    ...
    root.accept(new JavaRecursiveElementWalkingVisitor() {
      ...
      public void visitLiteralExpression(@NotNull PsiLiteralExpression literalExpression) {
        ...
        if (value != null &&
          ...
          // find MCBasicProperty for the given key in the project
          MCBasicProperty simpleProperty = ContainerUtil.getOnlyItem(MCBasicUtil.findProperties(project, key));
  ...
  /**
   * Gets the MCBasic Language 'value' string corresponding to the 'key'
   *
   ...
   *             defined by the MCBasic language file.
   */
  @Nullable
  @Override
  public String getPlaceholderText(@NotNull ASTNode node) {
    if (node.getPsi() instanceof PsiLiteralExpression psiLiteralExpression) {
      ...
      String key = text.substring(MCBasicAnnotator.SIMPLE_PREFIX_STR.length() +
          MCBasicAnnotator.SIMPLE_SEPARATOR_STR.length());

      MCBasicProperty simpleProperty = ContainerUtil.getOnlyItem(
          MCBasicUtil.findProperties(psiLiteralExpression.getProject(), key)
      );
      if (simpleProperty == null) {
