...
import me.kolterdyx.mcbasiclanguage.MCBasicCreatePropertyQuickFix;
...
final class MCBasicAnnotator implements Annotator {

  // Define strings for the MCBasic language prefix - used for annotations, line markers, etc.
  ...
  public void annotate(@NotNull final PsiElement element, @NotNull AnnotationHolder holder) {
    ...
    holder.newSilentAnnotation(HighlightSeverity.INFORMATION)
        .range(separatorRange).textAttributes(MCBasicSyntaxHighlighter.SEPARATOR).create();


    // Get the list of properties for given key
    String key = value.substring(SIMPLE_PREFIX_STR.length() + SIMPLE_SEPARATOR_STR.length());
    List<MCBasicProperty> properties = MCBasicUtil.findProperties(element.getProject(), key);
    if (properties.isEmpty()) {
      holder.newAnnotation(HighlightSeverity.ERROR, "Unresolved property")
          .range(keyRange)
          .highlightType(ProblemHighlightType.LIKE_UNKNOWN_SYMBOL)
          // ** Tutorial step 19. - Add a quick fix for the string containing possible properties
          .withFix(new MCBasicCreatePropertyQuickFix(key))
          .create();
    } else {
      // Found at least one property, force the text attributes to MCBasic syntax value character
      holder.newSilentAnnotation(HighlightSeverity.INFORMATION)
          .range(keyRange).textAttributes(MCBasicSyntaxHighlighter.VALUE).create();
    }
  }
