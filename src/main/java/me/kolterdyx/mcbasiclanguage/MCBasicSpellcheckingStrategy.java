...
final class MCBasicSpellcheckingStrategy extends SpellcheckingStrategy {

  @Override
  public @NotNull Tokenizer<?> getTokenizer(PsiElement element) {
    if (element instanceof PsiComment) {
      return new MCBasicCommentTokenizer();
    }

    if (element instanceof MCBasicProperty) {
      return new MCBasicPropertyTokenizer();
    }

    return EMPTY_TOKENIZER;
  }

  private static class MCBasicCommentTokenizer extends Tokenizer<PsiComment> {
  ...
  private static class MCBasicPropertyTokenizer extends Tokenizer<MCBasicProperty> {

    public void tokenize(@NotNull MCBasicProperty element, @NotNull TokenConsumer consumer) {
      //Spell check the keys and values of properties with different splitters
      final ASTNode key = element.getNode().findChildByType(MCBasicTypes.KEY);
      ...
      final ASTNode value = element.getNode().findChildByType(MCBasicTypes.VALUE);
